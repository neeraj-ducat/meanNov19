...
const Message = mongoose.model('messsages',{
    from: String,
    to: String,
    text: String,
    status: String
...
// Method to fetch all message sent by a user.
app.get('/messages/from/:mailId',function (req, res) {
    const criteria = req.params['mailId']
    Message.find({from: criteria}).then((result)=>{
        res.json(result);
    });
});
// Method to fetch all message sent to a user.
app.get('/messages/to/:mailId',function (req, res) {
    const criteria = req.params['mailId']
    Message.find({to: criteria}).then((result)=>{
        res.json(result);
    });
});
